...
const Properties = () => {
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    const bathroomsFilter =
      !bathroomsRange || features.bathRoom <= bathroomsRange;
    ...
    <>
      <div className="container mx-auto py-4 px-6 shadow-lg">
        <div className="flex text-right my-4">
          ...
          />
          <button className="hover:bg-primary bg-[#5994FF] text-white px-4 py-2 rounded">
            SEARCH
          </button>
          <button
            onClick={handleSearchReset}
            className="hover:bg-red-500 bg-red-400 text-white px-4 py-2 ml-2 rounded"
          >
            Reset
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <div>
              <label className="mb-2 flex items-center">
                Bedrooms ({bedroomsRange}-5)
              </label>
              <input
                onChange={(e) => setBedroomsRange(e.target.value)}
                value={bedroomsRange}
                type="range"
                min="0"
                max="5"
            ...
            <div>
              <label className="mb-2 flex items-center">
                Bathrooms ({bathroomsRange}-4)
              </label>
              <input
                onChange={(e) => setBathroomsRange(e.target.value)}
              ...
              <div key={filterName}>
                ...
                <label className="ml-2" htmlFor={filterName}>
                  {filterName.charAt(0).toUpperCase() +
                    filterName.slice(1).replace("has", "")}
                </label>
              </div>
            ))}
          </div>
          ...
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              ...
              className={`mx-1 px-4 py-2 rounded-lg ${
                currentPage === index + 1
                  ? "bg-[#5994D4] text-white"
                  : "bg-[#01204E] text-[#EBF4F6]"
              } hover:bg-[#5994D4]`}
